...
**Estado:** Implementado en `feature/expiration-type-config` (2026-09-21). Suite completa
verde (backend 559/559, frontend 942/942), sin errores nuevos de `tsc` en ninguno de los dos
lados. Sin desviaciones del plan original.
...
- [x] Migración simple (jsonb no, columna real):
```typescript
...
```
- [x] `task.entity.ts`: `@Column({ name: 'expiration_type', type: 'varchar', nullable: true, default: null }) expirationType: string | null;`
- [x] `cd backend && npx tsc --noEmit` sin errores nuevos.
- [x] Commit: `feat(tasks): agregar expiration_type para saber a qué tipo de vencimiento pertenece una tarea`
...
- [x] En `odoo-config.dto.ts`, agregar a `ExpirationTypeConfigEntryDto`:
```typescript
...
- [x] En `odoo-config.entity.ts`, extender el tipo inline de `expirationsTypeConfigs` con los
...
- [x] `cd backend && npx tsc --noEmit` — van a aparecer errores en `integration-config.service.ts`
  (los 4 tipos inline que replican esta forma) y en `expiration-tickets.service.ts` — se
  resuelven en las tasks siguientes.
- [x] Commit: `feat(integration-config): agregar taskName/defaultTimeMinutes/descripciones a ExpirationTypeConfigEntry`
...
- [x] Tests que fallan: `getOdoo()`/`getOdooConfigDecrypted()` devuelven los 4 campos nuevos
...
- [x] Implementar: extender los 4 tipos inline (`getOdoo`, `patchOdoo`'s cast,
...
- [x] `npx jest integration-config.service.spec --no-coverage` → pasan.
- [x] Commit: `feat(integration-config): propagar taskName/defaultTimeMinutes/descripciones en getOdoo/patchOdoo`
...
- [x] Test que falla: `findAll()` no incluye una entrada con `taskType: EXPIRATION_CONTROL`
...
no cambian.
- [x] Test collateral: si algún test existente asume `findAll()` devuelve N tipos, actualizar
  ...
  sumar el tipo).
- [x] Commit: `fix(task-config): excluir EXPIRATION_CONTROL de Admin → Mantenimientos`
...
- [x] Tests que fallan (agregar al `describe('createExpirationTicket', ...)` existente):
...
- [x] Implementar: renombrar `EXPIRATION_TYPE_LABELS` → `DEFAULT_EXPIRATION_TYPE_LABELS`;
...
- [x] `npx jest odoo.service.spec --no-coverage` → pasan.
- [x] Commit: `feat(odoo): nombre y descripción de ticket configurables por tipo de vencimiento`
...
- [x] Tests que fallan (agregar al `describe('closeTicket', ...)` existente, o crear uno si no
...
- [x] Implementar `resolveTimesheetDescription` como método privado (ver spec sección 6) y
  usarlo en `closeTicket`. Agregar el quinto parámetro opcional `expirationType?: string | null`.
- [x] `npx jest odoo.service.spec --no-coverage` → pasan.
- [x] Commit: `feat(odoo): resolver descripción de timesheet por tipo de vencimiento al cerrar el ticket`
...
- [x] Implementar: agregar `expirationType?: string | null` a los `params` de
  ...
  de `updateStatus`.
- [x] `npx jest tasks.service.spec --no-coverage` → pasan.
- [x] Commit: `feat(tasks): propagar expirationType a createFromExistingTicket y closeTicket`
...
- [x] Implementar `saveTypeConfig` (ver spec sección 2), eliminar `saveTypeConfigs` (bulk) y el
  DTO `PatchExpirationTypeConfigsDto` (queda huérfano). Actualizar los dos call sites señalados
  arriba.
- [x] `npx jest expiration-tickets.service.spec --no-coverage` → pasan.
- [x] Commit: `feat(notifications): guardar config de vencimientos por tipo y propagar nombre/tiempo/descripciones`
...
- [x] Tests que fallan: `PATCH /notifications/config/:type` requiere ADMIN (guard ya aplicado a
  nivel de clase, verificar que sigue); delega en `expirationTicketsService.saveTypeConfig(type, dto, user.email)`.
- [x] Implementar: reemplazar `@Patch() patch(@Body() dto: PatchExpirationTypeConfigsDto, ...)`
  por `@Patch(':type') patch(@Param('type') type: string, @Body() dto: ExpirationTypeConfigEntryDto, ...)`.
- [x] `npx jest notifications-config.controller.spec --no-coverage` → pasan.
- [x] `npx jest --no-coverage` (suite completa backend) → pasan.
- [x] `npx tsc --noEmit` → sin errores.
- [x] Commit: `feat(notifications): PATCH /notifications/config/:type reemplaza el guardado por blob completo`
...
- [x] Test que falla (`time-format.spec.ts`, archivo nuevo): `timeToMinutes('01:30')` → `90`;
...
- [x] Mover `TIME_PATTERN`, `minutesToTime`, `timeToMinutes` de
...
- [x] `npx ng test --include="**/time-format.spec.ts" --include="**/task-edit-dialog.component.spec.ts" --watch=false --browsers=ChromeHeadless` → pasan.
- [x] Commit: `refactor(shared): extraer time-format util desde task-edit-dialog`
...
- [x] Extender `ExpirationTypeConfigEntry` con los 4 campos nuevos (mismos nombres que el
  backend); extender `ExpirationDetail` con `defaultTimeMinutes: number | null`.
- [x] Test que falla: `patchTypeConfig(type, entry)` hace `PATCH /notifications/config/:type`
  con el `entry` completo como body.
- [x] Implementar `patchTypeConfig` en `NotificationsService`; eliminar `patchConfig` (bulk) —
  revisar que no quede ningún otro consumidor antes de borrarlo.
- [x] `npx ng test --include="**/notifications.service.spec.ts" --watch=false --browsers=ChromeHeadless` → pasan.
- [x] Commit: `feat(frontend): NotificationsService.patchTypeConfig reemplaza patchConfig`
...
- [x] Mirror exacto de `TaskEditDialogComponent`/`.html` en estructura, con estos campos en el
...
- [x] `data: { type: ExpirationType; entry: ExpirationTypeConfigEntry }` como
  `MAT_DIALOG_DATA` — el padre arma este objeto por fila.
- [x] `ngOnInit`: carga `getHelpdeskTeams()`/`getHelpdeskTags()` (ya existen en
  `IntegrationConfigService`) en paralelo (`forkJoin`), popula el form con `data.entry`.
- [x] `save()`: arma el `ExpirationTypeConfigEntry` completo desde el form, llama a
...
- [x] Tests (mismo nivel que `task-edit-dialog.component.spec.ts` si existe, o el nivel general
...
- [x] `npx ng test --include="**/notifications-type-edit-dialog.component.spec.ts" --watch=false --browsers=ChromeHeadless` → pasan.
- [x] Declarar el componente en `admin.module.ts` (`declarations`).
- [x] Commit: `feat(admin): NotificationsTypeEditDialogComponent — editor por tipo de vencimiento`
...
- [x] Reescribir `NotificationsConfigComponent`:
...
- [x] Reescribir el `.html`: `mat-table` de solo lectura (sin `[formGroupName]`), columna
...
- [x] Reescribir el `.spec.ts` acorde (va a cambiar sustancialmente — de "formulario reactivo +
  guardar todo" a "tabla + abrir diálogo por fila").
- [x] `npx ng test --include="**/notifications-config.component.spec.ts" --watch=false --browsers=ChromeHeadless` → pasan.
- [x] Commit: `refactor(admin): Vencimientos pasa a tabla de solo lectura + diálogo por tipo, igual que Mantenimientos`
...
- [x] Implementar: en `ngOnChanges`, separar el branch por tipo (ver spec sección "Task drawer");
  en `loadExpirationDetail`, sintetizar `this.taskConfig` desde `detail.defaultTimeMinutes` como
  se detalla en la spec.
- [x] `npx ng test --include="**/task-drawer.component.spec.ts" --watch=false --browsers=ChromeHeadless` → pasan.
- [x] Commit: `fix(task-drawer): tiempo predefinido de EXPIRATION_CONTROL viene del detalle por tipo, no de task-config`
...
- [x] Confirmar en Admin → Mantenimientos que `EXPIRATION_CONTROL` ya no aparece.
- [x] Confirmar en Admin → Vencimientos que la tabla es de solo lectura, el diálogo por tipo
